...
app.post('/loginadmin', function (req, res) {
  ...
  let usr = req.body
  connection.query(`SELECT * from admins where username="${usr.username}"`, function(err, rows, fields) {
    if (!err) {
      console.log('The solution is: ', rows[0]);

      if(typeof rows[0] != 'undefined' && pwVerify(usr.password, rows[0].password)){
...
app.post('/tambah-kategori',encoder, (req,res)=>{
  if(req.session.admin) {
    ...
    connection.query(`INSERT INTO kategori(nama_kategori,nama_kamar,harga_kamar) VALUES ('${nama_kategori}','${nama_kamar}','${harga_kamar}')`, (err,results)=>{
        if(err) throw err;
        res.redirect("/admin")
    });
} else {
...
app.post('/update-edit/:id_kategori',encoder, (req,res)=>{
  ...
  let data = {
      ...
      harga_kamar : req.body.harga_kamar
        ...
        connection.query('UPDATE kategori SET ? WHERE id_kategori = ?;' ,[data, id_kategori], function(err, result) {
            if (err) throw err;
    res.redirect('/admin')
  })
})
...
app.post('/tambah-databooking',encoder, (req,res)=>{
    ...
    connection.query(`INSERT INTO booking(nama_booking, alamat_booking, email_booking, notelp_booking, jumlah_hari) VALUES ('${nama_booking}','${alamat_booking}','${email_booking}', '${notelp_booking}','${jumlah_hari}')`, (err,results)=>{
        if(err) throw err;
...
app.get('/admin/:id_kategori',(req,res)=>{
    connection.query(`DELETE FROM kategori where id_kategori ='${req.params.id_kategori}'`,(req,rows)=>{
        res.redirect("/admin");
    })
})

app.get('/adminbooking/:id_booking',(req,res)=>{
    connection.query(`DELETE FROM booking where id_booking ='${req.params.id_booking}'`,(req,rows)=>{
